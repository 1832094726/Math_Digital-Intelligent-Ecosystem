...
#################### 以下为日志记录 ####################
# 全参Bert
model_name = 'BERT-ncf-mlp-final(lora)(' + nowTime + ')'
...
class CustomHead(PredictionHead):
    def __init__(self, model, head_name,**kwargs):
        ...
        self.config = {"layers": 1, "activation_function": None}
        self.build(model)

    def forward(self, outputs, cls_output=None, attention_mask=None, return_dict=False, **kwargs):

        outputs_linear = outputs

        return outputs_linear
...
class RecommendModel(nn.Module):
    def __init__(self):
        ...
        # BertAdapterModel.output_hidden_states = True
        ...
        # self.bert_rnn = AutoAdapterModel.from_pretrained(model_path, config=config)
        # config = LoRAConfig(r=128, alpha=16)
        # self.adapter_name = "lora_adapter"
        # self.bert_rnn.add_adapter(self.adapter_name, config=config)
        # self.bert_rnn.merge_adapter(self.adapter_name)
        # self.bert_rnn.active_adapters = self.adapter_name
        # self.bert_rnn.train_adapter(self.adapter_name)
        #
        # self.bert_rnn.register_custom_head("my_custom_head", CustomHead)
        # self.bert_rnn.add_custom_head(head_type="my_custom_head", head_name="custom_head")
        #
        # # 冻结BERT模型的参数，仅训练LoRA适配器的参数
        # for name, param in self.bert_rnn.named_parameters():
    ...
    def forward(self, user_data, item_data, question_similarity_vector, item_similarity_vector, T1 = 4, T2 = 4):
        ...
        usr_outputs = usr_outputs[:, 0, :]
        # usr_outputs= self.bert_rnn(
        #     input_ids=user_data_inputs["input_ids"],
        #     attention_mask=user_data_inputs["attention_mask"]
        # ).last_hidden_state[:, 0, :]
        ...
        item_outputs = item_outputs[:, 0, :]
        # item_outputs = self.bert_rnn(
        #     input_ids=item_data_inputs["input_ids"],
        #     attention_mask=item_data_inputs["attention_mask"]
        # ).last_hidden_state[:, 0, :]
