...
#################### 以下为日志记录 ####################
# 全参Bert
model_name = 'BERT-ncf-mlp-final-test(' + nowTime + ')'
...
##################### 以下为采样函数 #####################
def negative_sampling(user_item_matrix, other1_ratings_matrix, other2_ratings_matrix, user_dict, item_dict,
                      question_similarity_matrix, neg_ratio=1):
    """
...
########################### 以下为测试函数 ############################
def evaluate_model_on_test_set(state_dict, test_data, item_similarity_matrix,symbol_to_id, k, criterion,
                               batch_size=32):
    """
    ...
    # predictions = []
    enhanced_predictions = []  # 存储增强后的预测结果
    with torch.no_grad():  # 不需要计算梯度
        for i in range(0, len(test_user), batch_size):
            ...
            # 将预测结果reshape为(batch_size, num_items)
            current_batch_size = len(user_batch)
            # 找到每个问题中概率最大的预测
            batch_predictions = pred1.view(-1, len(set(test_item)))  # 重塑为 (batch_size, num_items)
            max_prob_indices = torch.argmax(batch_predictions, dim=1)  # 获取最大概率的索引

            # 获取对应的符号ID
            max_prob_symbol_ids = [list(symbol_to_id.values())[idx.item()] for idx in max_prob_indices]

            # 获取相似矩阵中对应的行
            # batch_predictions: (batch_size, num_items)
            # similarity_rows: (batch_size, num_items)
            similarity_rows = item_similarity_matrix[max_prob_symbol_ids]
            similarity_rows = torch.tensor(similarity_rows, dtype=torch.float).to(device)

            # 将原始预测与相似度信息相乘
            # enhanced_pred = batch_predictions * similarity_rows
            # enhanced_predictions.append(enhanced_pred.cpu())
            enhanced_batch_preds = []
            for j in range(current_batch_size):
                # 获取单个样本的预测向量和对应的相似度行
                pred_row = batch_predictions[j].unsqueeze(0)  # shape: (1, num_items)
                sim_row = similarity_rows[j]  # shape: (num_items,)

                # 计算预测向量与相似度行的点积
                enhanced_pred = torch.matmul(pred_row, sim_row)  # shape: (1, num_items)
                enhanced_batch_preds.append(enhanced_pred)

            # 将批次中的所有增强预测拼接起来
            enhanced_batch_predictions = torch.cat(enhanced_batch_preds, dim=0)  # shape: (batch_size, num_items)
            enhanced_predictions.append(enhanced_batch_predictions)

                # predictions = torch.cat(predictions, dim=0)  # (num_users * num_items,)
    enhanced_predictions = torch.cat(enhanced_predictions, dim=0)

    # 重塑 predictions 为二维张量 (num_users, num_items)
    # predictions = predictions.view(-1, len(set(test_item)))
    # enhanced_predictions = enhanced_predictions.view(-1, len(set(test_item)))

    # 计算测试损失
    test_loss = criterion(enhanced_predictions.view(-1), test_ratings)

    # 这里拿到sigmoid之后的概率后 先找到每一个问题中最大的predictions的id 然后保存预测最大概率的符号id 然后利用该符号id找到相似矩阵中的对应id的行数的信息 然后每一个问题的信息行同这个得到的行的信息相乘

    # 计算top-k准确率
    test_accuracy = calculate_top_k_accuracy(enhanced_predictions, test_user, test_item, test_ratings, k)

    return test_ratings, test_loss, test_accuracy, enhanced_predictions.view(-1)
...
class RecommendModel(nn.Module):
    ...
    def forward(self, user_data, item_data, question_similarity_vector, item_similarity_vector, T1=4, T2=4):
        ...
        user_data_inputs = self.tokenizer(user_data, return_tensors='pt', padding='max_length', truncation=True,
                                          max_length=320)
        # 移动相应的token到gpu上
        ...
        item_data_inputs = self.tokenizer(item_data, return_tensors='pt', padding='max_length', truncation=True,
                                          max_length=320)
...
if __name__ == "__main__":
    ...
    # 符号矩阵为132*132
    train_data = negative_sampling(train_ratings_matrix, emb_ratings_matrix, cluster_ratings_matrix, train_users_dict,
                                   train_items_dict, question_similarity_train_matrix, neg_ratio=1)
    ...
    train_user, train_item, train_ratings, train_emb_ratings, train_cluster_rating, question_similarity_vectors = zip(
        *train_data)
    ...
    for epoch in range(epochs):
        ...
        for i in range(0, len(train_user), batch_size):
            ...
            # 当前训练的相似矩阵数据移到GPU
            question_similarity_vector = torch.tensor(
                np.array(question_similarity_vectors[i:i + batch_size])).float().to(device)
            ...
            predictions1, predictions2, predictions3 = recommend_model(user_batch, item_batch,
                                                                       question_similarity_vector,
                                                                       item_similarity_vector, T1=T1, T2=T2)
    ...
    logging.info(
        f"Scale = 5, lamda1: {lambda_1}, lambda2: {lambda_2}, T1: {T1}, T2: {T2}, learning_rate_bert: {learning_rate_bert}, learning_rate_others: {learning_rate_others}, weight_decay: {weight_decay}, num_latent_factors: {num_latent_factors}, epochs: {epochs}, batch_size: {batch_size}, top_k: {top_k}")