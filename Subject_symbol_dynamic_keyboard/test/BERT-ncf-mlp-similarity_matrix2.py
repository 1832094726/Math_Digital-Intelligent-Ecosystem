...
#################### 以下为日志记录 ####################
# 全参Bert
model_name = 'BERT-ncf-mlp-similarity-matrix(' + nowTime + ')'
...
##################### 以下为采样函数 #####################
def negative_sampling(user_item_matrix, user_dict, item_dict, question_similarity_matrix, neg_ratio=1):
    """
    ...
    for user_id in range(num_users):
        ...
        positive_samples.extend(
            [(user_dict[user_id], item_dict[item_id.item()], 1, question_vector) for item_id in positive_items]
        )
        ...
        negative_samples.extend(
            [(user_dict[user_id], item_dict[negative_items[idx].item()], 0, question_vector) for idx in
...
class RecommendModel(nn.Module):
    ...
    def forward(self, user_data, item_data, question_similarity_vector, item_similarity_vector):
        ...
        predictions = torch.nan_to_num(predictions, nan=0.0)  # 将nan替换为0
        return predictions
...
if __name__ == "__main__":
    ...
    train_data = negative_sampling(train_ratings_matrix, train_users_dict, train_items_dict,
                                   question_similarity_train_matrix, neg_ratio=1)
    # breakpoint()
    ...
    train_user, train_item, train_ratings, question_similarity_vectors = zip(*train_data)
    ...
    # 3e-5 3e-4 50 20.39% 没收敛
    # 3e-3 3e-4 50 20.39% 没收敛
    ...
    for epoch in range(epochs):
        ...
        for i in range(0, len(train_user), batch_size):
            ...
            predictions = recommend_model(user_batch, item_batch, question_similarity_vector, item_similarity_vector)
        ...
        _, test_loss, test_accuracy, _ = evaluate_model_on_test_set(recommend_model, test_data,
                                                                    ...
                                                                    symbol_to_id,
                                                                     k=top_k,
                                                                    criterion=criterion, batch_size=32)
