...
test_data_path = "../../Datasets/change_2_add_test_layer_1_layer_2_layer_3.json"  # 测试集路径
...
def test_model_per_layer(model, layer_idx, batch_size, key=None, parent_label=None):
    ...
    # 根据层级和关键字构建模型文件名
    if layer_idx == 2:
        model_file_name = f"../Train Model_Change/layer2_classifier_{key}_if-idf_change_2.pth"
    elif layer_idx == 3:
        model_file_name = f"../Train Model_Change/layer3_classifier_{parent_label[0]}_{parent_label[1]}_if-idf_change_2.pth"
    else:
        model_file_name = f"../Train Model_Change/layer{layer_idx}_classifier_if-idf_change_2.pth"
...
if __name__ == '__main__':
    ...
    layer2_classifiers = {
        ...
        2: BertClassifier(model_path, labels_num=3, tfidf_features=1000).to(device),
    ...
    layer3_classifiers = {
        ...
        (2, 0): BertClassifier(model_path, labels_num=4, tfidf_features=1000).to(device),
